...
- **The flake still happens even with the full pattern.** Prompt rule 9 + wrapper-level retry handle normal occurrences, but we've seen clusters of 4+ consecutive flakes that exhaust both layers. For those, fall back to manual intervention from the caller's bash (e.g., direct file edits for trivial fixes). This was used successfully for the Story 0 biome.json schema fix.
