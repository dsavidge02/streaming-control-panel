...
**Previously: BETWEEN_STORIES** — Story 7 accepted 2026-04-18 via human visual verification (commit pending). Cumulative test count: **74 Vitest live (4 shared + 38 server + 32 client) + 17 Playwright baselines** (target; full server suite blocked locally by leftover `electron.exe` PID 944 holding `node_modules/.pnpm/electron@41.2.1` until `pnpm rebuild:node` retargets `better-sqlite3` for Node's NODE_MODULE_VERSION 137 — this is the expected post-`pnpm start` state, not a regression). Story 7 delivered: `apps/panel/server/src/electron/{app.ts,window.ts,preload.ts}` + `electron.vite.config.ts` at repo root + root `package.json` scripts (`start`, `dev`, `rebuild:electron`, `rebuild:node`; `postinstall electron-builder install-app-deps` removed per Finding 011 remediation discussion) + `apps/panel/server/src/index.ts` bootstrap (`startServer() → startElectron({ serverUrl })`) + `apps/panel/server/src/electron/readme.test.ts` (1 new Vitest for TC-3.5a) + README dev-mode table populated with three modes (client Vite, server Fastify, full Electron with HMR) and `NODE_MODULE_VERSION` troubleshooting note + `biome.json` includes regex corrected `!dist` → `!**/dist` (addresses the workspace-subdir gap Story 5 flagged for Story 8 — landed earlier than planned, net positive). Fix round triggered by Finding 011 (tech-design §Packaging empty preload slot invalid for electron-vite 5 — cascade: preload `rollupOptions.input` missing → renderer `rollupOptions.input` missing → root `package.json#main` missing; three distinct spec-shape defects caught by observed-run gate across one story cycle; dual review passed each intermediate state because spec compliance was perfect). Finding 012 (transient wrapper exit 127, originally filed as background-shell PATH gap, corrected same day: `codex` IS on PATH in background bash subshells; exit 127 was stale-process or startup-race). **Bonus post-acceptance cleanup:** `apps/panel/client/src/components/ErrorEnvelopeCard.tsx:44` `<pre>` class list swapped `overflow-x-auto` → `whitespace-pre-wrap break-all leading-relaxed`; eliminates horizontal-scrollbar eyesore the user observed on the 500 `SERVER_ERROR` envelope render during Story 7 observed-run; ui-spec §346 "pre-formatted 10px raw envelope" intent preserved; no test impact (`SignInButton.test.tsx` asserts on `toHaveTextContent`, not layout). **Verification state (local, pre-commit):** `pnpm red-verify` green (96 files format-checked, 97 linted, 3 packages typecheck clean); `pnpm --filter @panel/client --filter @panel/shared test` → 36/36 green; `pnpm --filter @panel/server exec vitest run src/electron/readme.test.ts` → 1/1 green; server suite blocked pending `rebuild:node` (see above). Observed-run acceptance: human-driven (TC-1.1b / TC-3.1a app launch + TC-3.2a HMR + TC-9.3a); user visually verified app renders landing at `app://panel` with Fastify on `:7077`. Prior accepted-and-committed: Story 0 (`15b3c00` + `7c0446d`) + Story 1 (`e65d7f5`) + Story 2 (`0e51796`) + Story 3 (`b4d578f`) + Story 4 (`9457d11`) + Story 5 (`4220836`) + Story 6 (`2040a42`). Ready for Story 8 (packaging: electron-builder + launches-to-landing verification) after commit.

**Previously: STORY_ACTIVE** — Story 7 started 2026-04-18 in a fresh Claude Code session (post-`/clear`). Phase: `reviewing` (impl GREEN iter 1; wrapper verdict 13 REAL_ERROR but ground-truth gate exit 0 — driver correctly treated gates as authoritative; cumulative test count: 74 Vitest live = 4 shared + 38 server + 32 client; 17 Playwright unchanged; impl thread `019d9ee2-3b62-77c2-a5ba-fb942c7b35db`). Team `epic-1-app-shell` persisted from prior session (config.json had only team-lead; no stale members to prune). `ls-team-impl-v2` skill reloaded before spawn. Entering with: 73 Vitest live (4 shared + 37 server + 32 client) + 17 Playwright baselines; `.red-ref` rotated to Story 6's SHA `2040a42`. Story 7 scope: Electron main process (`server/src/electron/{app.ts,window.ts}`), `app://` privileged-scheme registration at module load, electron-vite config at repo root, root `package.json` `start`/`dev` scripts → `electron-vite dev`, README dev-mode table populated (three modes — client, server, start), new `server/src/electron/readme.test.ts` (TC-3.5a grep assertion). Target delta: +1 Vitest server test (73→74 live), zero Playwright baselines. Driver gate: `pnpm red-verify && pnpm verify`. Post-commit: rotate `.red-ref` + `pnpm green-verify`. Dependencies satisfied: Stories 3 (data layer), 4 (server gate + Origin), 6 (renderer router) all accepted. Observed-run TCs (TC-1.1b, TC-3.1a, TC-3.2a, TC-9.3a) require human verification via `pnpm start` before acceptance — this is the first story with observed-run requirements beyond Playwright baselines.

**Previously: BETWEEN_STORIES** — Story 6 accepted and committed 2026-04-17 in a fresh Claude Code session. Cumulative test count: **73 Vitest live (4 shared + 37 server + 32 client) + 17 Playwright baselines** (unchanged — redirect states already captured in Story 5 via testBypass). Story 6 delivered the client-side gate in 1 driver iteration: new files `src/app/{defineRoute.ts, routes.ts, router.tsx, RequireAuth.tsx}`, new placeholder views `src/views/{HomePlaceholder,SettingsPlaceholder}.tsx`, new 5-test file `src/app/router.test.tsx` (TC-2.1a/b/c + TC-2.5b + TC-2.6a); `src/App.tsx` MODIFY swapped `<Landing />` for `<RouterProvider router={router} />` preserving `<PaletteProvider>` wrapper. Dual review clean: 1 Minor (RequireAuth missing Epic 2 replacement comment from tech-design) accepted-risk and filed for pre-epic-verification cleanup. Incidental bonus: Codex's App.tsx rewrite removed an erroneous UTF-8 BOM that Story 5's implementer prepended (filed cleanup resolved naturally). Impl thread `019d9ea3-a22e-71e3-ba95-6ca535e15978`; review thread `019d9eac-5b3d-7010-9aa6-ad37f229be63`. Delivery markers: impl=2, review=3. Zero walls, zero relaunches. `.red-ref` rotated to Story 6's commit SHA post-commit; `pnpm green-verify` confirmed green. Prior accepted-and-committed: Story 0 (`15b3c00` + `7c0446d`) + Story 1 (`e65d7f5`) + Story 2 (`0e51796`) + Story 3 (`b4d578f`) + Story 4 (`9457d11`) + Story 5 (`4220836`).
...
### Story 7 — Acceptance Receipt (2026-04-18, user-driven visual verification)

**Deviation from prior-story pattern:** Story 7 did not run the full dual-review harness pass at orchestrator level — observed-run is the dominant gate for this story (TC-1.1b, TC-3.1a, TC-3.2a, TC-9.3a) and the user performed the visual verification directly. Codex implementer drove the code deliverables + fix rounds via `ls-team-impl-v2` harness through Finding 011's three cascading spec-shape defects; after the third defect landed (`package.json#main`), `pnpm start` succeeded and the user visually confirmed the app renders the Story 5 landing via `app://panel` with Fastify on `:7077` and renderer HMR working. User then flagged the `<ErrorEnvelopeCard>` visual defect (ugly horizontal scrollbar on raw-envelope `<pre>`) and requested log update + Story 8 transition.

**Deliverables (untracked files pending commit):**

- `apps/panel/server/src/electron/app.ts` (new) — `startElectron({ serverUrl })`, `protocol.registerSchemesAsPrivileged` for `app` scheme at module load (per tech-design-server §Electron Shell), `protocol.handle('app', ...)` via `net.fetch(pathToFileURL(...))` after `app.whenReady()`
- `apps/panel/server/src/electron/window.ts` (new) — `createMainWindow()` 1280×800 min 960×600, `contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`; dev-path via `ELECTRON_RENDERER_URL` env, prod-path `app://panel/index.html`
- `apps/panel/server/src/electron/preload.ts` (new) — one-line stub: `// Epic 1: preload slot reserved for Epic 2+ IPC; intentionally empty.` (Finding 011 Option A remediation — entry exists so electron-vite 5 rollup accepts the slot)
- `apps/panel/server/src/electron/readme.test.ts` (new) — 1 Vitest (TC-3.5a) greps README for three commands + ports
- `electron.vite.config.ts` at repo root (new) — `main` + `preload` + `renderer` sections, each with explicit `rollupOptions.input`; renderer uses `@vitejs/plugin-react` + `@tailwindcss/vite`; `@panel/shared` alias resolves to `apps/panel/shared/src` in both main and renderer configs; renderer root `apps/panel/client` with outDir `../server/dist/renderer`
- `apps/panel/server/src/index.ts` MODIFY — replaced `export {};` with `await startServer(); await startElectron({ serverUrl: server.url })`; top-level await is the Electron main-process bootstrap entry
- `package.json` MODIFY — added top-level `"main": "apps/panel/server/dist/main/index.js"` (Finding 011 remediation); added scripts `start`, `dev`, `rebuild:electron`, `rebuild:node`; added devDeps `@tailwindcss/vite`, `@vitejs/plugin-react`, `electron-vite`; removed `postinstall electron-builder install-app-deps`
- `biome.json` MODIFY — `"!dist"` → `"!**/dist"` (workspace-subdir fix; Story 5 post-mortem predicted this for Story 8; landed earlier, ships ahead)
- `README.md` MODIFY — dev-mode table rows fleshed out: `pnpm --filter @panel/server dev` → "Fastify standalone on `http://127.0.0.1:7077`"; `pnpm start` → "Full Electron app - Fastify (`:7077`) + Vite renderer (`:5173`) + main process with HMR"
- `docs/v2-findings/011-tech-design-preload-slot-invalid-electron-vite.md` (new) — three-occurrence cascade tally
- `docs/v2-findings/012-codex-harness-background-shell-path-gap.md` (new) — original PATH-gap hypothesis + same-day corrected diagnosis (transient exit 127)
- `docs/v2-findings/README.md` MODIFY — index rows for 011 + 012
- `docs/epic-1-app-shell/error-log.txt` (new, untracked) — user's `pnpm start` session captures including the NODE_MODULE_VERSION ABI error that drove the `rebuild:electron` / `rebuild:node` dual-script design

**Top findings and dispositions:**

| # | Finding | Severity | Disposition | Reasoning |
|---|---------|----------|-------------|-----------|
| 1 | Tech-design §Packaging prescribes an empty `preload` slot (no rollup input) for electron-vite 5 — rejected by electron-vite's validator | Critical (caught by observed-run, not by structural review) | **fixed in-story** via Finding 011 Option A (stub `preload.ts` entry file + explicit `rollupOptions.input`); v2-findings entry filed for upstream skill refinement | Dual review passed the spec-literal config; only `pnpm start` surfaced the defect. Spec-accuracy issue, not review-depth. |
| 2 | Tech-design §Packaging omits renderer `rollupOptions.input` — same class as #1 | Critical | **fixed in-story**; cascaded into Finding 011 tally | Electron-vite 5 requires explicit rollup input for every configured section. |
| 3 | Tech-design §Packaging omits root `package.json#main` | Critical | **fixed in-story**; cascaded into Finding 011 tally | electron-vite's `ensureElectronEntryFile` check requires it; neither `pnpm verify` nor dual review exercises this code path. |
| 4 | `ErrorEnvelopeCard` raw-envelope `<pre>` uses `overflow-x-auto` → horizontal scrollbar on long JSON strings; visually ugly at 500 error sizes | Minor (visual-behavior) | **fixed post-acceptance** (`overflow-x-auto` → `whitespace-pre-wrap break-all leading-relaxed`); user-reported during observed-run | Spec §346 says "pre-formatted" — `<pre>` still satisfies; overflow treatment was not specified. Zero test impact. |
| 5 | Finding 012 original diagnosis (background-shell PATH gap) incorrect | Minor | **corrected-in-place** same day | Harness-side PATH guard recommendation remains as defense-in-depth; not blocking. |

**Exact gate commands run locally (pre-commit, 2026-04-18):**

- `pnpm red-verify` → exit 0 (96 files format, 97 lint, 3 packages typecheck clean)
- `pnpm --filter @panel/client --filter @panel/shared test` → exit 0 (32 + 4 = 36 green)
- `pnpm --filter @panel/server exec vitest run src/electron/readme.test.ts` → exit 0 (1/1 green — new Story 7 test)
- Full `pnpm verify` server-suite run → blocked locally: `electron.exe` PID 944 holds `node_modules/.pnpm/electron@41.2.1`, preventing `pnpm rebuild:node` from retargeting `better-sqlite3` to NODE_MODULE_VERSION 137; expected post-`pnpm start` state, not a regression. Unblock path: close the running Electron window → `pnpm rebuild:node` → `pnpm verify` should land 74/74.

### Story 7 Orchestration Observations (captured for v2-findings + future handoffs)

1. **Observed-run is a dominant gate for Electron stories.** Three distinct tech-design-§Packaging spec-shape defects (preload input, renderer input, `package.json#main`) each passed dual review + `pnpm verify` cleanly. Each was caught only by `pnpm start` attempting to boot. This cascades Finding 010's pattern (visual-behavior defects) into a second category: toolchain-config-validation defects. Both strengthen the case for a "run the prescribed toolchain once against the prescribed config" step in tech-design verification.

2. **Dual review is not a substitute for running the tool.** Codex spec-compliance + Opus architectural both graded Story 7's config passes as compliant — because they were, perfectly faithful to the document. The document carried the defect. Any future epic with novel toolchain config (Vite, electron-builder, webpack, etc.) should include a "tool accepts this config" verification gate before calling tech-design done.

3. **Story 7's biome.json `!dist` → `!**/dist` fix landed the Story 8 mitigation early.** Memory note `feedback_empirical_build_pollutes_gate.md` cited this as a Story 8 risk; Story 7's implementer independently identified and fixed it during config work. Net positive — Story 8 entering with clean format gate against workspace dist directories.

4. **User-driven observed-run acceptance is a valid shortcut when Playwright+Vitest cover structural state.** The formal harness pattern (dual-review + orchestrator gate rerun + coverage audit) was not exercised for Story 7's acceptance at the log level. The code deliverables went through the implementer's own Codex dual-review cycle during Finding 011's fix rounds; orchestrator confirmed red-verify + the subset of Vitest that doesn't require native-module rebuild. For stories where the dominant gate is human observation (Electron boot, real-device interaction), this compression is pragmatic — just document the pre-commit Vitest suite state honestly rather than claiming a 74/74 pass that wasn't fully re-run.

5. **ErrorEnvelopeCard bonus cleanup pattern continues (Story 6 app.tsx BOM, Story 7 `<pre>` overflow).** When the user surfaces a visual defect during observed-run that's outside story scope but touches a file already in the story-touched set, fix it in the same uncommitted batch. Cheap, and avoids the "we noticed this but it's not in this story" queue growing indefinitely.

### Story 7 — Transition Checkpoint (accepted 2026-04-18; commit pending)

**Cumulative test count after Story 7: target 74 Vitest live + 17 Playwright baselines.** Breakdown: 4 shared (Stories 0+4) + 38 server (Stories 1-4 + Story 7's readme.test.ts) + 32 client (Stories 5+6). Locally confirmed: 37/74 (all client + shared + new Story 7 readme test); remaining 37 server tests depend on `pnpm rebuild:node` retargeting `better-sqlite3`'s NODE_MODULE_VERSION — blocked by leftover electron.exe as noted above. Story 8 target: packaging gate + launches-to-landing verification (test-plan Chunk 8); cumulative floor entering Story 8 is ≥ 74 Vitest live + 17 Playwright after local unblock + green-verify.

**Problems encountered:**

1. Finding 011 cascade — three tech-design §Packaging defects detected only by observed-run (`preload.rollupOptions.input`, `renderer.rollupOptions.input`, root `package.json#main`). All three fixed in-story via stub preload entry + explicit inputs + package.json main-field; upstream skill refinement filed.
2. Finding 012 transient `wrapper exit 127` misdiagnosed as background-shell PATH gap — corrected same-day after verifying `codex` IS resolvable in background bash subshells; real cause was stale-process or startup-race.
3. `<ErrorEnvelopeCard>` raw-envelope `<pre>` horizontal-scrollbar visual defect — user-caught during observed-run; fixed post-acceptance in the same uncommitted batch.
4. NODE_MODULE_VERSION ABI mismatch between Electron and Node — by-design post-`pnpm start` state; README documents the unblock (`pnpm rebuild:node`). Not a regression.

**Impact and resolution:** all four absorbed in-cycle. Problem 1 is Finding 011 — primary v2-findings deliverable of Story 7 (observed-run-catches-structurally-invisible pattern extended to toolchain configs). Problem 2 is Finding 012 — defense-in-depth recommendation stands but root cause corrected. Problem 3 is a post-acceptance bonus cleanup (Story 6 pattern). Problem 4 is expected behavior, documented.

**Recommendations for Story 8 (packaging: electron-builder + launches-to-landing verification):**

- Story 8 consumes Story 7's `electron-vite` build output (`apps/panel/server/dist/{main,preload,renderer}`). Confirm no further §Packaging spec-shape defects; if electron-builder's `files`/`directories.output`/`asar` shape has similar defect risk, apply the Finding 011 lesson: run `electron-builder build` once against the prescribed config during tech-design verification.
- Full-suite verification requires `pnpm rebuild:node` first on any machine where `pnpm start` has been run (Electron ABI lingers). Document this in the story's DoD if not already.
- `biome.json` `!**/dist` fix already landed in Story 7 — Story 8's generated artifacts (`apps/panel/server/dist/**`, electron-builder output paths) should be format-gate-clean without additional include tweaks.
- Story 8 re-verifies Story 5's visual baselines — the packaged binary launches to the landing view. The Playwright fullPage-default fix from Story 5 (Finding 010) must hold; verify no regression when capturing from the packaged app if that path is exercised.
- Pre-commit for Story 7 must land before Story 8 starts: commit the ~13 uncommitted files (README/index.ts/biome.json/package.json/pnpm-lock.yaml/team-impl-log.md/v2-findings/README.md + electron/{app,preload,window,readme.test}.ts + electron.vite.config.ts + ErrorEnvelopeCard.tsx + 011 + 012). `braindump.md`, `.claude/`, `docs/epic-1-app-shell/error-log.txt` are workspace-local scratch and should stay untracked (or be filed under `.gitignore` if not already).

**Boundary inventory after Story 7:**

| Boundary | Status | Owning Story | Change from Story 6? |
|----------|--------|--------------|----------------------|
| Electron shell | **integrated** (main process bootstrap via `apps/panel/server/src/index.ts` top-level await; `app://panel` privileged scheme at module load; `BrowserWindow` 1280×800 with contextIsolation + sandbox; `ELECTRON_RENDERER_URL` dev path vs `app://` prod path; `electron-vite` dev server with HMR; `rebuild:electron`/`rebuild:node` scripts for ABI swap) | Story 7 | **advanced** (was not started) |
| Renderer asset delivery | **integrated** via `app://panel` + `net.fetch(pathToFileURL(...))` | Story 7 | new surface |
| React Router | wired (Story 6) | Story 6 | — |
| Client renderer | fully implemented (Stories 5+6; Story 7's observed-run re-verifies under Electron native chrome) | Story 5 + 6 + 7 (observed-run) | — |
| Twitch OAuth (`/auth/login`) | stub body + real Origin gate | Story 2 → Story 4 → Epic 2 | — |
| Twitch OAuth callback (`/oauth/callback`) | stub body + exempt gate | Story 2 → Epic 2 | — |
| Live SSE producers | stub cadence + real session gate | Story 2 → Story 4 → Epic 4a | — |
| SQLite filesystem | integrated | Story 3 | — |
...
| OS keychain (`keytar`) | not started | Epic 2 | — |
| GitHub Actions CI | partial (docs-fallback) | Story 9 | — |
| Packaged binary | not started | Story 8 | — (next) |

### Story 8 — Pre-Acceptance Receipt (2026-04-18)

**CLI evidence references** (6 Codex sessions across impl + review + 3 fix rounds + cleanup; plus orchestrator direct-file edits for diagnostic revert):

- **Impl iter 1** — thread `019da1ed-de08-7191-9155-803696c2b516`; JSONL `codex-story-8-impl.iter-1.jsonl`. Driver exit 0 iter 1/4. Delivered the narrow-spec config (electron-builder.yml + package script + readme-package.test.ts + README Packaging section) + ran `pnpm package` 20× inside the Codex session producing an NSIS installer. Wrapper verdict REAL_ERROR but ground-truth gate GREEN per §Invariant 6.
- **Review** — thread `019da207-f536-7402-b799-b2ab674af5f2`; JSONL `codex-story-8-review.iter-1.jsonl`; findings at `codex-story-8-findings.md`. BLOCK verdict (hollow asar: 11KB with 3 files, no renderer + no node_modules + no app.asar.unpacked). Reviewer dispatched the fix driver themselves via `drive-until-green.sh` per skill §Step 5.
- **Fix round 1** — thread `019da21a-c54b-7501-8e0a-a3fa06e352aa`; JSONL `codex-story-8-fix.iter-1.jsonl`. Driver exit 0 iter 1/4. Routed via the reviewer. Fixed C1 (Story 7 latent renderer outDir defect `"../server/dist/renderer"` → `path.resolve(__dirname, "apps/panel/server/dist/renderer")`), C2 (electron-builder + pnpm node_modules traversal via route-2 = 7 runtime deps at root package.json), updated tech-design-server.md §Packaging, justified `name`/`version` additions. Expanded gate now verified asar contents (500KB floor + asar.unpacked + .node binding + index.html grep).
- **Fix round 2 (ABI)** — thread `019da274-df58-70a0-9407-eb331a8faaf6` iter 1 + iter 2. **DRIVER ABANDONED** by orchestrator after gate-design flaw detected: `pnpm verify` before `pnpm package` fails because `pnpm package` leaves binding at Electron ABI 145 and Vitest needs Node ABI 137 (two-direction ABI swap). Codex invented `scripts/package-and-restore-native.mjs` (rebuild:electron → build → builder → delay(5s) → rebuild:node → touch) + `scripts/touch-better-sqlite3-binding.mjs` (pnpm-store binding mtime refresh) to square the circle. Orchestrator verified fix via corrected gate sequence (`pnpm rebuild:node && red-verify && verify && package`) + a senior-engineer subagent ran `pnpm format:fix` to clean biome indentation.
- **Fix round 4 (consolidated: hoisting + drizzle + smoke gate)** — thread `019da29a-44e6-7451-8bbe-5cac22d91f25`; JSONL `codex-story-8-fix-4.iter-2.jsonl`. Driver exit 0 iter 2/4. Applied the 5-agent-researched consolidated fix: `.npmrc` `node-linker=hoisted` (research-root-cause), `electron-builder.yml` + `apps/panel/server/drizzle/**` (Drizzle migrations in asar), `scripts/smoke-packaged.mjs` boot-verify gate using child-process + HTTP poll (`/auth/login` 501 NOT_IMPLEMENTED as "server alive" sentinel), `scripts/patch-local-asar-cli.mjs` (Windows asar-list backslash → forward-slash fix for gate grep), incidental Story 7 defect fix in `app.ts` RENDERER_ROOT path (`../../renderer` → `../renderer`). Kept fix-2's swap-cycle script because hoisting did NOT retire @electron/rebuild's pnpm-store write-back issue on this host.
- **Fix round 5 (whenReady TLA hang)** — diagnostic senior-engineer added `[diag]` logging to `index.ts`/`app.ts`/`window.ts` writing to `%TEMP%/panel-debug.log` and `%APPDATA%/streaming-control-panel/debug.log`. Fresh packaged build + user observed-run produced log sequence stopping at `[diag] startElectron: awaiting whenReady` — Fastify bound port 7077, but `await app.whenReady()` in the TLA chain never resolved. Fix: rewrote `apps/panel/server/src/index.ts` to use idiomatic `app.whenReady().then(async () => { ... })` pattern (register ready callback synchronously at module load, not through TLA), added `app.requestSingleInstanceLock()` for double-launch protection, removed redundant `await app.whenReady()` inside `startElectron`. **This is the fix that made TC-4.2a pass.**
- **Cleanup** — senior-engineer reverted diagnostic logging; landed deferred doc updates (architecture.md, CLAUDE.md, test-plan.md, 00-foundation.md retrospective sidecar) + decisions-log.md Decision #11 (whenReady TLA hang); fixed `scripts/smoke-packaged.mjs` Windows tree-kill bug (`child.kill()` left Electron helper processes orphaned holding port 7077 → switched to `taskkill /F /T /PID <pid>`).

Delivery marker `DELIVERY_CHECK_MARKER_ECHO_7077` observed across every Codex JSONL. Walled sessions: 0 (all cleared on first flake retry).

**Top findings and dispositions:**

| # | Finding | Severity | Disposition | Reasoning |
|---|---------|----------|-------------|-----------|
| C1 | Hollow asar (renderer + node_modules missing, 11KB) | Critical | **fixed** fix-1 | Story 7 latent renderer outDir defect + narrow `files:` list. Fixed via absolute path.resolve + route-2 (7 runtime deps at root). |
| C2 | Native ABI mismatch (better-sqlite3 Node 137 vs Electron 145) | Critical | **fixed** fix-2 | @electron/rebuild's `npmRebuild: true` logs "finished" but doesn't write-back to pnpm's hoisted store. Replaced with explicit `pnpm rebuild:electron` prepend + `npmRebuild: false` + post-package swap back to Node ABI for Vitest. |
| C3 | Drizzle migrations missing from asar | Critical | **fixed** fix-4 | Added `apps/panel/server/drizzle/**` to `electron-builder.yml` `files:` (Pattern C from research — works inside asar via Electron's patched fs). |
| C4 | `app.whenReady()` TLA chain hang in packaged main | Critical | **fixed** fix-5 | Rewrote index.ts to `app.whenReady().then(...)` pattern. Root cause: ESM+TLA+Electron ready-event interaction bug specific to packaged builds. |
| M1 | `signAndEditExecutable: false` not in spec | Major | **fixed (keep + document)** | Empirically necessary on Windows hosts without SeCreateSymbolicLink privilege (winCodeSign .dylib extraction fails). Inline YAML comment landed; tech-design-server.md §Packaging updated. |
| M2 | Story 7 latent renderer outDir defect (`..`-relative path) | Major | **fixed** fix-1 | Absolute `path.resolve(__dirname, "apps/panel/server/dist/renderer")`. Documented as Decision #2 in decisions-log. |
| M3 | Story 7 latent RENDERER_ROOT defect in app.ts (`../../renderer` → `../renderer`) | Major | **fixed** fix-4 | Surfaced only in packaged mode (dev served renderer from Vite dev server, never touched RENDERER_ROOT path). |
| M4 | smoke-packaged.mjs `child.kill()` doesn't clean Electron helper process tree on Windows | Major | **fixed** cleanup | Switched to `taskkill /F /T /PID <pid>`. Would have orphaned processes every CI run. |
| m1 | `name`/`version` root additions beyond append-only scope | Minor | **justified, retained** | Required by electron-builder for app identity resolution (NSIS installer version, productName fallback). |
| m2 | 7 root runtime deps added in fix-1 as pnpm-symlink workaround | Minor | **retained (optional future removal)** | Under `node-linker=hoisted`, these MAY be removable, but fix-4 didn't test removal — kept for stability. Deferred to post-M3 cleanup. |
| m3 | `@electron/rebuild` redundant in devDependencies (bundled by electron-builder 26.8+) | Minor | **defer to post-M3** | Advisory warning only. Our explicit `rebuild:electron` script uses it directly; removal requires switching to electron-builder's bundled rebuild. |
| m4 | electron-builder advisory: description + author missing from package.json | Minor | **defer to post-M3** | Non-blocking packaging warnings. |

Zero findings remain open-blocking.

**Exact gate commands run by orchestrator** (ground truth, 2026-04-18 ~19:50 local):

- `pnpm red-verify` → exit 0 (100 files format-check, 101 linted, 3 packages typecheck clean)
- `pnpm verify` → exit 0 (75 Vitest: 4 shared + 39 server + 32 client)
- `pnpm verify-all` → exit 0 (75 Vitest + 17 Playwright baselines all matching, 17.8s)
- `pnpm package` → exit 0 (NSIS installer `Streaming Control Panel Setup 0.0.0.exe` 104 MB)
- `pnpm smoke:packaged` → **PASS** (boot-verify gate: spawns packaged exe, polls `127.0.0.1:7077/auth/login`, asserts 501 NOT_IMPLEMENTED envelope, cleans up via Windows `taskkill /F /T`)
- `grep -rn '"link:' apps/*/package.json package.json` → exit 2 / 0 matches (no dep-graph regression)
- `git diff --stat` across all touched paths: 13 modified + 7 new (see checkpoint below). No overwrite regressions.

**TC-4.2a observed-run (human verification):** PASSED 2026-04-18 after fix round 5. User installed via `dist/packaged/Streaming Control Panel Setup 0.0.0.exe`, launched "Streaming Control Panel" from Start Menu, renderer window appeared with Story 5 landing view + Twitch sign-in button.

**UI spec compliance:** Story 8 is partial UI-spec scope (re-verifies Story 5's 17 baselines via packaged artifact observed-run). No new Playwright baselines produced. Story 5's visuals remain authoritative; `pnpm verify-all` confirms all 17 still match pixel-perfect.

**Open risks:** none blocking. Post-M3 queue (electron-builder advisories, @electron/rebuild dep removal, root-package runtime deps potential removal under hoisted) tracked in decisions-log.md Decisions #9-#10 and the Standing TODOs list.

### Story 8 Orchestration Observations (captured for v2-findings + future handoffs)

1. **Observed-run-only defect pattern is structural, not anecdotal.** Four distinct Criticals (hollow asar → ABI mismatch → missing drizzle → whenReady hang) all passed `pnpm red-verify` + `pnpm verify` + dual review + structural artifact-shape checks. Every one only surfaced when the USER launched the installed artifact. This class of defect is invisible to dev-path gates. The cure is the `pnpm smoke:packaged` gate delivered in fix-4 — a Playwright-electron-equivalent boot-verify that launches `win-unpacked/<AppName>.exe` and polls a server route. This gate catches 3 of the 4 defect classes mechanically; the fourth (whenReady TLA hang) was caught because the smoke gate actually exercises the app start path. **Finding 013 writeup is the primary v2-findings deliverable of Story 8.**

2. **Research-first would have saved 6+ hours.** User asked "if mdv works, how hard is it to replicate?" at ~4 hours into the fix cycle. Five parallel research agents then identified `node-linker=hoisted` as the ROOT cause of the pnpm-symlink traversal class (3 distinct defects symptomatic of it). Had this been done BEFORE fix-1, Story 8 would have been a 1-iteration story. The retrospective lesson: for novel toolchain configs (packaging, release engineering, CI), doing community-pattern research + finding a reference repo BEFORE the first Codex spawn is high-leverage. Decision #1 documents this as a process finding.

3. **Story 7 carried 3 latent defects that only surfaced in Story 8.** (i) electron-vite renderer `..`-relative outDir silently fails on Windows; (ii) `app.ts` RENDERER_ROOT was `../../renderer` instead of `../renderer` (off-by-one in the `dist/main/` → `dist/renderer/` path); (iii) `await app.whenReady()` was part of the TLA chain that hangs in packaged mode. Story 7's DoD used `pnpm start` for observed-run — dev mode doesn't exercise `electron-vite build`, doesn't read RENDERER_ROOT, and doesn't go through the same packaged TLA path. Cross-cutting lesson for future stories with Electron: **if the artifact format is expected to support packaging, test the packaged artifact, not just dev mode.** Story 8's smoke gate now provides this for all future stories.

4. **Tree-kill is a real Windows gap in Node child_process.** Electron spawns 4 helper processes (main + GPU + renderer + utility). `child.kill()` on Windows SIGTERM-equivalent only kills the parent. Left 3 orphans holding port 7077 after every smoke run. Fixed by `execFileSync("taskkill", ["/F", "/T", ...])` — Windows-specific but necessary. Cross-cutting for any future story that spawns GUI processes.

5. **`--remote-debugging-port` + no child processes = app.whenReady never firing = Chromium browser init stalled.** Diagnostic trick worth remembering: if Electron's remote-debugging port doesn't open despite the flag being passed, Chromium itself isn't initializing, not a secondary issue. Points at main-thread blocking or TLA+ready interaction.

6. **Cleanup discipline: diagnostic logging should have a revert commit planned from day one.** The `[diag]` logging added during fix-5 diagnosis was uncommitted + required a cleanup pass. Having a pre-approved "diagnostic revert" template (wrapping `[diag]` calls in `if (process.env.PANEL_DEBUG)` for example) would make emergency instrumentation low-cost to leave in. Minor process improvement for future epics.

7. **Decisions log earned its keep.** 11 decisions captured across research synthesis + fix rounds. Without this artifact, Epic 2+ would likely re-discover hoisting, smoke gates, and Drizzle-migrations-in-asar from scratch. The log is now a first-class Epic 1 deliverable alongside team-impl-log.md and the v2-findings corpus.

### Story 8 — Transition Checkpoint (accepted 2026-04-18; ready to commit)

**Cumulative test count after Story 8: 75 Vitest live + 17 Playwright baselines + 1 boot-verify gate.** Breakdown: 4 shared (Story 0's 3 codes + Story 4's 1 gateExempt) + 39 server (Stories 1-4 + Story 7's readme.test.ts + Story 8's readme-package.test.ts) + 32 client (Story 5's 27 + Story 6's 5). Story 9 target: CI workflow + `tools/ci/workflow.test.ts` (2 tests per test-plan Chunk 9); cumulative floor entering Story 9 is ≥ 75 Vitest live + 17 Playwright + smoke gate.

**Problems encountered:** 4 Criticals surfaced by observed-run (detailed above), 3 Story 7 latent defects (renderer outDir, RENDERER_ROOT path, whenReady TLA), 1 gate-design conflict (two-direction ABI swap), 1 Windows tree-kill bug (smoke gate orphans), 1 scope-expansion dropped by Codex mid-flight (4 doc updates missed in fix-4, landed in cleanup).

**Impact and resolution:** all absorbed in-cycle across 5 fix rounds + cleanup. Story 8 ships green with the strongest verification surface of the epic (dev gates + Playwright baselines + boot-verify smoke gate). The `.npmrc` `node-linker=hoisted` fix is an architectural correction that Epic 2+ inherits. The decisions-log is now the canonical reference for cross-cutting concerns surfaced during Epic 1.

**Recommendations for Story 9 (GitHub Actions CI workflow):**

- Story 9 delivers `.github/workflows/ci.yml` that runs `pnpm verify` on PRs. Per tech-design and `test-plan.md` Chunk 9, it intentionally SKIPS `pnpm verify-all` (no Playwright) and SKIPS `pnpm smoke:packaged` (no Electron runtime on CI Ubuntu runner + no way to exercise a GUI app there without xvfb complexity). Just `pnpm install && pnpm verify` + lockfile-frozen.
- Story 9 may add 2 Vitest tests to `tools/ci/workflow.test.ts` per the chunk spec — asserting the workflow file contents parse + contain specific fields. Target: 77 Vitest live.
- Under hoisted pnpm, CI caching should use the root `node_modules/` directory + `pnpm-lock.yaml` hash. Actions like `pnpm/action-setup@v4` + `actions/setup-node@v4` with `cache: 'pnpm'` handle this.
- Consider a SECOND Story 9-deferred workflow for packaging: a manual-dispatch workflow that runs `pnpm package` + `pnpm smoke:packaged` for release verification. Out of scope for Story 9 MVP but worth flagging in its DoD comments.
- No .red-ref rotation needed for Story 9 gate; it's a standard red → green cycle.
- Story 9 is the **last story of Epic 1**. After acceptance: set state → `PRE_EPIC_VERIFY`, run Epic-Level Verification (four parallel reviews + meta-reports + synthesis per skill §Epic-Level Verification).

**Boundary inventory after Story 8:**

| Boundary | Status | Owning Story | Change from Story 7? |
|----------|--------|--------------|----------------------|
| Packaged binary | **integrated** (NSIS installer produces bootable app; TC-4.2a observed-run verified; `pnpm smoke:packaged` gate automated) | Story 8 | **advanced** (was not started) |
| pnpm workspace layout | **hoisted** (`.npmrc` `node-linker=hoisted`; flat `node_modules/`; electron-builder traversal works natively) | Story 8 | architectural shift |
| Boot-time verification surface | **integrated** (`pnpm smoke:packaged` spawns `win-unpacked/<AppName>.exe`, polls `/auth/login`, verifies 501 NOT_IMPLEMENTED envelope) | Story 8 | new surface |
| Electron shell | integrated | Story 7 | — |
| Renderer asset delivery | integrated via `app://panel` + `net.fetch(pathToFileURL(...))` | Story 7 | — |
| React Router | wired (Story 6) | Story 6 | — |
| Client renderer | fully implemented (Stories 5+6+7 observed-run + Story 8 packaged observed-run) | Story 5 + 6 + 7 + 8 | re-verified packaged |
| Twitch OAuth (`/auth/login`) | stub body + real Origin gate | Story 2 → Story 4 → Epic 2 | — |
| Twitch OAuth callback (`/oauth/callback`) | stub body + exempt gate | Story 2 → Epic 2 | — |
| Live SSE producers | stub cadence + real session gate | Story 2 → Story 4 → Epic 4a | — |
| SQLite filesystem | integrated (packaged artifact + unpacked .node binding via asarUnpack) | Story 3 + Story 8 packaging | — |
| Session cookies (`iron-session`) | wired (unseal active; issue path Epic 2) | Story 4 | — |
| Twitch Helix / EventSub | not started | Epic 3 / Epic 4a | — |
| OS keychain (`keytar`) | not started | Epic 2 | — |
